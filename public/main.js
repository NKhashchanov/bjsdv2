...
class Profile {
  ...
  static register(userData, callback) {
    ...
    static transferMoney({ to, currency, amount }, callback) {
      ...
      return ApiConnector.transferMoney( {to, amount, currency}, (err, data) => {
    ...
    static addUserToFavorites({ id, name }, callback) {
        ...
        return ApiConnector.addUserToFavorites( {id, name}, (err, data) => {
            console.log(`Adding ${name}`);
            callback(err, data);
        });
    }
    ...
    static removeUserFromFavorites({ id }, callback) {
        ...
        return ApiConnector.removeUserFromFavorites( {id}, (err, data) => {
            console.log(`Removing ${id}`);
            callback(err, data);
        });
    }
